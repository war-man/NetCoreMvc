var productCategoryController = function () {
    ...
    function registerEvents() {
        ...
        $('#frmMaintainanceSelectProductCategory').valid({
            errorClass: 'red',
            ignore: [],
            lang: 'vi',
            rules: {
                ddlProductCategoryId: { required: true },
            }
        });
        ...
        $('#btnSaveProductCategory').on('click', function (e) {
            if ($('#frmMaintainanceSelectProductCategory').valid()) {
                ...
                var parentId = $('#ddlProductCategoryId').combotree('getValue');

                //$('input[name=ddlParentPostCategory]').change(function () {
                //    parentId = $('#ddlParentPostCategory').val();
                //});               
                setValueToNewProductCategory(parentId);   
                //activeShowHome();
                //activeShowHot();
            }
        });
    ...
    function save() {
        ...
        $.ajax({
            ...
            success: function () {
                aspnetcore.notify('Cập nhật danh mục sản phẩm thành công', 'success');
                $('#modal-add-edit').modal('hide');
                resetFormMaintainance();
                aspnetcore.stopLoading();
                loadData(true);
            ...
            error: function () {
                aspnetcore.notify('Cập nhật danh mục sản phẩm có lỗi xảy ra', 'error');
                aspnetcore.stopLoading();
            }
        });
    ...
    function resetFormMaintainance() {
        ...
        $('#lblParentProductCategoryName').text('');
        $('#txtName').val('');
        $('#txtUrl').val('');
        $('#txtCode').val('');        
        initTreeDropDownCategory('');
    ...
    function fillData(that) {       
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#lblParentProductCategoryName').text(data.ParentPostCategoryName);                                
    ...
    function initTreeDropDownCategory(selectedId) {
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#ddlProductCategoryId').combotree({
                    data: arr
                });
                if (selectedId != undefined) {
                    $('#ddlProductCategoryId').combotree('setValue', selectedId);
                }
            }
        });
    }

    function setValueToNewProductCategory(parentId) {
        $.ajax({
            url: "/Admin/ProductCategory/PrepareSetNewProductCategory",
            type: 'POST',
            dataType: 'json',
            data: {
                parentId: parentId
            ...
            success: function (response) {
                ...
                $('#lblParentProductCategoryName').text(response.parentName);                
    ...
    function activeShowHome() {
        $('#ckShowHome').change(function () {
            if (this.checked) {
                $.ajax({
                    url: "/Admin/ProductCategory/SetNewHomeOrder",
                    type: 'GET',
                    dataType: 'json',
                    success: function (response) {
                        $('#txtHomeOrder').val(response.homeOrder);
    ...
    function activeShowHot() {
        $('#ckShowHot').change(function () {
            if (this.checked) {
                $.ajax({
                    url: "/Admin/ProductCategory/SetNewHotOrder",
                    type: 'GET',
                    dataType: 'json',
                    success: function (response) {
                        $('#txtHotOrder').val(response.hotOrder);
